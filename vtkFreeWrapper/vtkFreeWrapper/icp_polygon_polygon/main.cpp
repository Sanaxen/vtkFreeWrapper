...
int main(int argc, char** argv)
{
	if (argc < 2)
	{
		printf("icp_polygon_polygon.exe -s obj_file_src -t obj_file_tgt [-i Iterations  -landmark sample_landmarks(0..1|1..N) -tol tolerance] -o output\n");
		return -99;
	}
	...
	int sample_landmarksN = sample_landmarks*std::min(polygon1->Get()->GetOutput()->GetNumberOfPoints(), polygon2->Get()->GetOutput()->GetNumberOfPoints());
	...
	int stat = expoter->exportVertexColorOBJ(output, poly_output, meshFilter->GetPolyMapper(), meshFilter->GetPolyActor());
